...
import DoctorSearch from "@/components/dashboard/patient/DoctorSearch";
import AIRecommendationResult from "@/components/dashboard/patient/AIRecommendationResult";
import FirstAidGuide from "@/components/dashboard/patient/FirstAidGuide";
...
import GetMedicines from "@/components/dashboard/patient/GetMedicines";
        ...
        <main className="flex-1 overflow-y-auto p-8">
          {activeSection === "dashboard" && (
            <div>
              ...
              {/* AR First Aid Guide Promo Button */}
              <div className="bg-gradient-to-br from-teal-500 to-emerald-600 rounded-3xl p-6 sm:p-8 mb-8 text-white shadow-lg relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6 cursor-pointer hover:shadow-xl transition-shadow" onClick={() => navigate('/ar-first-aid')}>
                <div className="absolute right-0 top-0 w-64 h-64 bg-white/10 rounded-full blur-3xl" />
                <div className="relative z-10">
                  <h2 className="text-2xl font-bold mb-2">AR First Aid Guide</h2>
                  <p className="text-teal-50 max-w-xl">
                    Experience interactive step-by-step first aid instructions overlaid directly on your camera feed. Learn how to apply bandages, treat burns, and more!
                  </p>
                </div>
                <button 
                  onClick={(e) => { e.stopPropagation(); navigate('/ar-first-aid'); }}
                  className="relative z-10 shrink-0 bg-white text-teal-600 font-bold px-6 py-3 rounded-xl shadow-md hover:bg-slate-50 transition-colors"
                >
                  Start AR Guide
                </button>
              </div>

              <DoctorSearch onBookClick={handleBookDoctor} />
              ...
              <div className="mb-8 mt-12">
                <div className="bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
                  <div className="relative z-10">
                    <h2 className="text-2xl font-bold tracking-tight mb-2">Find Doctor by Symptoms</h2>
                    <p className="text-fuchsia-50 text-base max-w-xl">
                      Not sure which specialist to see? Describe your symptoms and our AI assistant will find the right medical expert for you.
                    </p>
                  </div>
                </div>
                <AIRecommendationResult 
                  onBookClick={handleBookDoctor} 
                  fetchDoctorsByCategory={async (category: string) => {
                    ...
                    const doctorsData = querySnapshot.docs.map(doc => {
                      ...
                      return {
                        id: doc.id,
                        name: d.Name || d.name || "Unknown Doctor",
                        specialization: d.Specialization || d.specialization || "Specialist",
                        experience: d.Experience ? `${d.Experience} Years` : d.experience || "N/A",
                        consultationFee: d['Consultation Fee'] || d['Consultaion Fee'] || d['Consulation Fee'] || d.consultationFee || 0,
                        available: true,
                        availabilityText: d.Availability || d.Avaiblity || d.availability || "Not Specified",
                        rating: d.Rating || d.Ration || d.rating || 0,
                        slots: d.slots || ["09:00 AM", "11:00 AM", "01:00 PM", "04:00 PM"],
                        hospital: d.hospital || "Arogix Clinic",
                        color: d.color,
                        avatar: d.avatar
                      };
                    }) as any[];
                    ...
                    return doctorsData.filter(d => 
                      d.specialization?.toLowerCase() === category.toLowerCase()
                    );
                  }} 
                />
              </div>
          ...
          {activeSection === "medicines" && <GetMedicines />}
