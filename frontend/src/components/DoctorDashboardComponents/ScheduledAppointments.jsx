...
const ScheduledAppointments = ({ appointments, onStartConsultation }) => {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">{appointments.length} confirmed today</p>
        </div>
              ...
              <div key={appt.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50/60 transition-colors duration-150">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-slate-800 text-sm truncate">{appt.name}</p>
